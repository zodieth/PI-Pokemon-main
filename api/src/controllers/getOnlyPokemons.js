...
const firstApiPetition = async () => {
  try {
    const Api = await axios
      ...
      .then((data) => {
        return c; // guardo todo la informacion de cada pokemon con todos su datos en mi variable Api
      });
    console.log(Api);
...
const GetAllPokemon = async () => {
  const firstRequestInformation = await firstApiPetition();
  //   const secondRequestInformation = await SecondApiRequest();
  //   const AllInf = firstRequestInformation.concat(secondRequestInformation);
  //   return AllInf;
  return firstApiPetition;
};
